...
Admin gets the whole page: add accounts, reset passwords, change roles,
delete, and assign product access. Manager gets the Product Access tab
only - assigning products is a scheduling decision, but creating accounts
and changing roles is not, and a Manager who could edit roles could make
themselves an Admin.
"""
...
st.title("👤 Users")
st.caption("**Admin** — full access everywhere. **Manager** — can edit any product's "
           "Operation Time, Cleaning Time, and Actual Temperature on **BMR Master** "
           "(Operation text, Equipment IDs, Standard Temperature, add/delete products and "
           "uploads stay Admin-only), schedule any product, edit ECR cleaning templates on "
           "**ECR Master**, assign product access on the tab below, and "
           "reschedule/pause/resume/delete batches on **Batches** — plus everything a "
           "Planner can do. Managers cannot add accounts, reset passwords, change roles or "
           "delete users. **Planner** — schedule batches and look up any batch's BMR/ECR "
           "output at any time, plus the equipment map/timeline; no edit or delete rights "
           "anywhere.")
...
st.divider()
if is_admin:
    tab_add, tab_manage, tab_access = st.tabs(
        ["Add User", "Reset Password / Role / Delete", "Product Access"]
    )
else:
    # Manager: product assignment only, no account administration.
    tab_add = tab_manage = None
    tab_access = st.container()
...
with tab_access:
    st.caption("Scope which products a **Manager** is responsible for on the **Batches** page: "
               "once assigned, that Manager can only reschedule, pause/resume and delete batches "
               "for those products. It does not limit what they can schedule or edit — every "
               "Manager can start a batch for any product on the Scheduler and edit any recipe "
               "on BMR Master. Everyone can view the Equipment Map and Gantt Timeline for the "
               "full schedule. Admins and Planners are never scoped.")
